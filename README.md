...
You can execute the test suite from the command‑line after installing dependencies:
...
A successful run will report all passing cases.

To exercise the same behaviour in a clean container, build and run the provided Docker image:
...
The Dockerfile installs dependencies from `package.json` and invokes `npm test` when the container starts, matching the evaluation requirements.
